...
def get_dataset(
    ...
    allowed_datasets = ['TimeSeries', 'STFT', 'HUNT4Masked',
                        'USCHAD', 'PAMAP2', 'MobiAct', 'MultiPairSTFT']
...
class MultiPairSTFTDataset(HARDataset):
    """Dataset for spectrogram-based HAR with multi-sensor pairing support."""
    ...
    def __init__(self, args,
        ...
        args (dict): Dataset specific parameters
            Needs to include:
                ...
                normalize (bool): Whether to normalize spectrograms
                phase (bool): Include phase
                pairing_strategy (str): 'bilateral', 'ipsilateral', or 'diagonal'
        ...
        self.window = torch.hann_window(self.n_fft)
        self.phase = args['phase']
        # Pairing strategy for multi-sensor setup
        self.pairing_strategy = args['pairing_strategy'] if 'pairing_strategy' in args else 'bilateral'
        # Time domain split
        assert args['sequence_length'] > self.hop_length, \
                print('sequence_length < hop_length not allowed')
        self.sequence_length = args['sequence_length'] // self.hop_length -1
    ...
    def __getitem__(self, idx):
        ...
        # Split x into pair1 and pair2 based on pairing strategy
        # After STFT, features are stacked: [time, freq_bins * num_channels]
        # where freq_bins = n_fft // 2 + 1
        num_freq_bins = self.n_fft // 2 + 1

        # Define which channels belong to each sensor
        # Assuming x_columns order: [RW_x, RW_y, RW_z, LW_x, LW_y, LW_z,
        #                             RL_x, RL_y, RL_z, LL_x, LL_y, LL_z]
        pairing_strategy = self.pairing_strategy if hasattr(self, 'pairing_strategy') else 'bilateral'

        if pairing_strategy == 'bilateral':
            # Pair1: RW + LW (channels 0-5), Pair2: RL + LL (channels 6-11)
            pair1_channels = list(range(0, 6))
            pair2_channels = list(range(6, 12))
        elif pairing_strategy == 'ipsilateral':
            # Pair1: RW + RL (channels 0-2, 6-8), Pair2: LW + LL (channels 3-5, 9-11)
            pair1_channels = list(range(0, 3)) + list(range(6, 9))
            pair2_channels = list(range(3, 6)) + list(range(9, 12))
        elif pairing_strategy == 'diagonal':
            # Pair1: RW + LL (channels 0-2, 9-11), Pair2: LW + RL (channels 3-5, 6-8)
            pair1_channels = list(range(0, 3)) + list(range(9, 12))
            pair2_channels = list(range(3, 6)) + list(range(6, 9))
        else:
            raise ValueError(f"Unknown pairing_strategy: {pairing_strategy}")

        # Convert channel indices to feature indices in stacked STFT
        pair1_indices = []
        for ch in pair1_channels:
            pair1_indices.extend(range(ch * num_freq_bins, (ch + 1) * num_freq_bins))

        pair2_indices = []
        for ch in pair2_channels:
            pair2_indices.extend(range(ch * num_freq_bins, (ch + 1) * num_freq_bins))

        pair1 = x[:, pair1_indices]
        pair2 = x[:, pair2_indices]
